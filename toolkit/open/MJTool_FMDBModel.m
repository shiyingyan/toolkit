...
- (BOOL)save
{
    ...
    MJTool_FMDBGetDatabase *ykDB = [MJTool_FMDBGetDatabase shareTool_FMDBGetDatabase];
    __block BOOL res = NO;
...
+ (BOOL)saveObjects:(NSArray *)array
{
    ...
    for (MJTool_FMDBModel *model in array) {
        if (![model isKindOfClass:[MJTool_FMDBModel class]]) {
            return NO;
        }
    }
    
    __block BOOL res = YES;
    MJTool_FMDBGetDatabase *jkDB = [MJTool_FMDBGetDatabase shareTool_FMDBGetDatabase];
    // 如果要支持事务
...
//- (BOOL)deleteObject
//{
//    MJTool_FMDBGetDatabase *jkDB = [MJTool_FMDBGetDatabase shareMJTool_FMDBGetDatabase];
//    __block BOOL res = NO;
//    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
//        NSString *tableName = NSStringFromClass(self.class);
...
+ (NSArray *)findByCriteria:(NSString *)criteria
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            MJTool_FMDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
